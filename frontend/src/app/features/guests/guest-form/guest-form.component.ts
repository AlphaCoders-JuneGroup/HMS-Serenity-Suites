...
export class GuestFormComponent implements OnInit {
    ...
    this.guestService.getGuestById(this.guestId!).subscribe({
      next: (res) => {
        ...
        this.form.patchValue({
          ...
          loyaltyTier: g.loyaltyTier || 'Regular',
          preferredRoomType: g.preferences?.preferredRoomType || '',
          pillowType: g.preferences?.pillowType || '',
          dietaryNeeds: g.preferences?.dietaryNeeds || '',
          specialNeeds: g.preferences?.specialNeeds || '',
          ...
          emergencyRelation: g.emergencyContact?.relation || '',
          companyName: g.company?.name || '',
          companyBillingEmail: g.company?.billingEmail || '',
          companyTaxId: g.company?.taxId || '',
          marketingOptIn: !!g.marketingOptIn,
          dataProcessingConsent: g.dataProcessingConsent !== false,
          dateOfBirth: g.dateOfBirth ? String(g.dateOfBirth).slice(0, 10) : '',
          anniversary: g.anniversary ? String(g.anniversary).slice(0, 10) : '',
        });
  ...
  onSubmit(): void {
    ...
    const payload = {
      ...
      loyaltyTier: v.loyaltyTier || 'Regular',
      preferences: {
        preferredRoomType: v.preferredRoomType || '',
        pillowType: v.pillowType?.trim() || '',
        dietaryNeeds: v.dietaryNeeds?.trim() || '',
        specialNeeds: v.specialNeeds?.trim() || '',
      },
      isBlacklisted: !!v.isBlacklisted,
      blacklistReason: v.blacklistReason?.trim() || '',
      emergencyContact: {
        name: v.emergencyName?.trim() || '',
        phone: v.emergencyPhone?.trim() || '',
        relation: v.emergencyRelation?.trim() || '',
      },
      company: {
        name: v.companyName?.trim() || '',
        billingEmail: v.companyBillingEmail?.trim() || '',
        taxId: v.companyTaxId?.trim() || '',
      },
